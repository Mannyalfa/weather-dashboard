...
function getWeather(q) {
    ...
    $.ajax({
        ...
        var cityMain1 = $("<div col-8>").append($("<p><h2>" + response.name + ' (' + currentDate + ')' + "</h2><p>"));
    ...
    $.ajax({ // gets the 5 day forecast API
        ...
        for (var i = 0; i < arrayList.length; i++) {
            if (arrayList[i].dt_txt.split(' ')[1] === '12:00:00') {
                ...
                var date5 = $("<h5>").text(response.list[i].dt_txt.split("MMMM-dd-yyyy")[0]);
                var image = $('<img>').attr('src', 'http://openweathermap.org/img/w/' + arrayList[i].weather[0].icon + '.png');
                var degreeMain = $('<p>').text('Temp : ' + arrayList[i].main.temp + ' °F ('+ celsius + '°C)');               
